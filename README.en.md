...
| [tool-plugin.md](skills/dsh-plugin-dev/references/tool-plugin.md) | defineTool contract, parameter schema, output modes, and the rc.7 settings-card keyed-slot contract |
...
| dsh (npm) | **npm @deepseek-ai/dsh@0.1.0-rc.7 (lib production mode)** | Started via `npx -p @deepseek-ai/dsh@0.1.0-rc.7 dsh web` (lib production mode; do not `install -g` globally) |
...
| `~/.dsh/source/current` | → DSH 0.1.0-rc.7 (npm) — a leftover of the snapshot-junction era (does not exist under npm mode) |
...
Startup: `npx -p @deepseek-ai/dsh@0.1.0-rc.7 dsh web` (npm 0.1.0-rc.7, lib production mode; do not `install -g` globally). The old snapshot-era wrapper is deprecated: `~/.local/bin/dsh` (do not run `bin/dsh` directly — on Windows, MSYS path conversion triggers `ERR_UNSUPPORTED_ESM_URL_SCHEME`, issue #388; the wrapper launches tsx with a `file://` URL to work around it).
