...
RC1 的 client graph、附件、commands、settings 和品牌规则以官方 npm `next` 包 README/类型及 `deepseek-ai/deepseek-harness` 已实现 notes 为准。当前官方 npm 元数据显示 `@deepseek-ai/dsh-client-web-react@next` 与 `@deepseek-ai/dsh-client-schema-form@next` 仍是 0.1.0-rc.7；不要将它们写成 RC1。

RC1 client bundles 由 `exports["./client"]` 进入 lazy-CJS graph；动态依赖用精确 `dsh.client.external` specifier，provider 先于 consumer，跨包值导入走 `/client`。
...
- 坑 1 / 1b / 3 / 5（vendor cordis 双副本、junction、monorepo tsc 路径）**仅适用于 monorepo/旧 snapshot 场景**；
  npm 路径（`@deepseek-ai/dsh@0.1.1-rc.1`）下：peer 全为 scoped（`@deepseek-ai/cordis`），`npm install` 即得唯一 Cordis 身份与 devDependencies 工具链，无需 junction；
