...
以 dsh-tool-csv 为模板（组织内 8 个插件同模式，均验证可用）。
...
1. **统一 JSON 文本输出**：多 action 工具（csv/regex/time 都是）返回 JSON 字符串，schema 保持 `{ type: 'string' }`——模型消费等价，实现与测试显著简化；
2. **错误用 throw**：`execute` 抛 `new Error('xxx: 原因')`；**实测：dsh 工具管道把工具失败作为 error content block 返回（`ctx.tools.execute` 不 reject）**，schema 校验失败（未知 action 等）也走同一路径——driver 断言按 content 文本核对；
3. **确定性**：纯函数、无副作用、无网络——参数会记入会话日志，工具描述中明确警告敏感输入。
