...
- git push 用 **SSH**（`git@github.com:`，id_rsa 已注册）比 HTTPS 可靠（HTTPS 间歇 `Connection reset`）；新仓库首次推送要 `git push -u origin main`（否则报 "no upstream branch"）。
...
- **实测现状**：组织内 8 个工具插件实际**都没打 `marisa-plugin` topic**（`gh api .../topics` 全空）；hub 收录实际靠 `catalog.source.json` 登记（手动或等 2 小时自动同步）——topic 机制未在本组织验证，按"可选"描述，别写进验收门槛；
