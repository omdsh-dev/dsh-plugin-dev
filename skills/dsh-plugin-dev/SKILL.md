...
一句话：**验证过的插件形态 = 一个 cordis 插件包 + 一份 `cordis.patch.yml`（bundle 形态），挂进 profile 就生效。**
...
1. **选形态** → [references/overview.md](references/overview.md)
   固定使用 bundle 形态（cordis 包 + patch，官方机制）。registry 与 skill 形态的情况记录在 overview 里。
...
| git push 间歇 `Connection reset` | HTTPS 通道不稳 | 用 SSH（`git@github.com:`，id_rsa 已注册）；新仓库 `git push -u origin main` |
